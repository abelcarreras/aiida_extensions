...
#            self.append_to_report('{}'.format(dos.tolist()))
 ...
 #       self.add_result('data', data)
        ...
        for wf in wf_list:
            ...
            if int_neg/int_pos > 1e-6:
                volume = wf.get_result('final_structure').get_cell_volume()
                self.append_to_report('Volume {} not suitable for QHA (imaginary frequencies in DOS), skipping it'.format(volume))
                continue
 ...
 #           cv.append(wf.get_result('thermal_properties').get_array('cv'))
            ...
            fe_phonon.append(np.interp(temperature_fit, temperatures, wf.get_result('thermal_properties').get_array('free_energy')))
            entropy.append(np.interp(temperature_fit, temperatures, wf.get_result('thermal_properties').get_array('entropy')))
